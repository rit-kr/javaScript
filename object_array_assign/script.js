...
for(let member of got.houses[1].people) {
  console.log(`I am ${member.name} and my bio is ${member.description}`);
}
...
for(let member of got.houses[1].people) {
  if(member.name == "Tywin Lannister") {
    console.log(`I am ${member.name} and my bio is ${member.description}`);
  }
}
...
// for(let member of got.houses[0].people) {
//   allDescription.push(member.description)
// }
...
for (let elment of allDescription) {
  if (elment.length > 30)  {
    console.log(elment);
  }
}